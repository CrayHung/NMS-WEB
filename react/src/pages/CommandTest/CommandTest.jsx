...
/** 全畫面 Loading Overlay（灰色背景＋讀取旋轉圖） */
const LoadingOverlay = ({ show, text = 'Waiting for device response...' }) => {
    if (!show) return null;
    return (
        <div
            style={{
        ...
        >
            ...
            >
                ...
                <span style={{ fontWeight: 600, whiteSpace: 'nowrap' }}>{text}</span>
            </div>
        </div>
    );
};
...
const CommandTest = () => {
    ...
    const handleRfPowerComplete = (data) => {
        if (pendingRequestRef.current &&
            ...
            setRfPowerData(data)
            setLoading(false)
            setMessage({ type: 'success', text: 'RF Power data collection complete' })
            pendingRequestRef.current = null
        }
    }
    ...
    const sendCommand = async () => {
        ...
        try {
            switch (commandType) {
                case 'status': await amplifierAPI.queryStatus(selectedDevice); break
                case 'settings': await amplifierAPI.querySettings(selectedDevice); break
                case 'model': await amplifierAPI.queryModelInfo(selectedDevice); break
                case 'rf-power': await amplifierAPI.queryAllRFPower(selectedDevice); break
                default: throw new Error('Unknown command type')
            }

            setMessage({ type: 'info', text: `Command sent, waiting for device response...` })
            ...
            console.error('Failed to send command:', error)
            setLoading(false)
            setMessage({ type: 'danger', text: `Failed to send command: ${error.message}` })
            pendingRequestRef.current = null
        }
    }
    ...
    const handleDeviceChange = (deviceEui) => {
        ...
        setRfPowerData(null)
        setMessage(null)
        if (deviceEui) loadCurrentStatus(deviceEui)
    }
    ...
    // （仍保留你的 RF Power chart 工具函式，供未來需要時使用）
    const renderRfPowerChart = () => {
        if (!rfPowerData) return null
        const chartData = []
        const frequencies = new Set()
        ...
        sortedFreqs.forEach(freq => {
            const point = { frequency: freq }
            Object.values(rfPowerData.inputPower || {}).forEach(part => {
                if (part[freq] !== undefined) point.inputPower = part[freq]
            })
            Object.values(rfPowerData.outputPower || {}).forEach(part => {
                if (part[freq] !== undefined) point.outputPower = part[freq]
            })
            if (point.inputPower !== undefined && point.outputPower !== undefined) {
                point.gain = point.outputPower - point.inputPower
            }
        ...
        return (
            <div>
                ...
                <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={chartData}>
                        <CartesianGrid strokeDasharray="3 3" />
                        <XAxis dataKey="frequency" label={{ value: 'Frequency (MHz)', position: 'insideBottom', offset: -5 }} />
                        <YAxis label={{ value: 'Power (dBmV)', angle: -90, position: 'insideLeft' }} />
                        <Tooltip />
                        <Legend />
                        <Line type="monotone" dataKey="inputPower" stroke="#8884d8" name="Input Power" strokeWidth={2} dot={false} />
                        <Line type="monotone" dataKey="outputPower" stroke="#82ca9d" name="Output Power" strokeWidth={2} dot={false} />
                        <Line type="monotone" dataKey="gain" stroke="#ffc658" name="Gain" strokeWidth={2} dot={false} />
                    </LineChart>
                </ResponsiveContainer>
            </div>
        )
                    ...
                    <Card.Body>
                        ...
                        <Row className="mb-0">
                            <Col>
                                <div className="d-flex gap-4 text-muted">
                                    {requestTime && (
                                        <span><FiClock className="me-1" />Sent: {formatDateTime(requestTime)}</span>
                                    )}
                                    {responseTime && (
                                        <span><FiCheck className="me-1 text-success" />Received: {formatDateTime(responseTime)}</span>
                                    )}
                                    {requestTime && responseTime && (
                                        <span>Duration: {((responseTime - requestTime) / 1000).toFixed(1)} s</span>
                                    )}
                                </div>
                            </Col>
                        </Row>
